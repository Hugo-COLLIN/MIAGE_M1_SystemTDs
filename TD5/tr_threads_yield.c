...
void * thread_function(void * arg)
{
	...
	// laisser un peu de temps pour que d'autres threads soient créés
	sleep(1);
	...
	for (i = 0; i < 500000; i ++){
		if (i%100000 == 0){
			fprintf(stderr, "Thread %ld cède le processeur à %ld\n", id, time(NULL));
			// Le thread appelant cède le CPU. 
			// Le thread est déplacé à la fin de la liste des processus prêts de sa priorité.
			// Un autre thread sera exécuté.			
			sched_yield();
		}
		for (j = 0; j < 10000; j ++);
	}
